...
# Now supports 4-column input (angle, dist, x, y) and draws rectangular safety zone overlay.
...
# ---------------- CONFIG ----------------
SERIAL_PORT = "/dev/cu.usbmodem2101"            # "AUTO" or explicit, e.g., "COM5" or "/dev/tty.usbmodemXXXX"
BAUD = 115200
MAX_RANGE_MM = 1200
...
# Rectangular safety zone (match Arduino constants, in mm)
RECT_X_MIN = -200
RECT_X_MAX = 800
RECT_Y_MIN = -100
RECT_Y_MAX = 600
...
def main():
    ...
    if port == "AUTO":
        ...
        else:    print("❌ Auto-detect failed. Set SERIAL_PORT manually.")
    ...
    scale = max_r_px / MAX_RANGE_MM  # mm -> px scale for rectangle
    ...
    while running:
        ...
        if connected:
            try:
                ...
                if line:
                    parts = line.split(",")
                    # handle both 2- and 4-column input
                    if len(parts) >= 2:
                        ang = float(parts[0])
                        dist = float(parts[1])
                        if dist >= 0:
                            deg_bin = int(round(ang))
                            deg_bin = clamp(deg_bin, ANGLE_MIN, ANGLE_MAX)
        ...
        # --- Draw safety rectangle (same as Arduino zone) ---
